...
export const FileViewer = ({ fileURI, title, className = "" }) => {
    if (!fileURI) return null;

    // IPFS URI handling (backward compatibility)
    if (fileURI.startsWith("ipfs://")) {
        const hash = fileURI.replace("ipfs://", "");
        ...
        const isPDFType = mimeType === "application/pdf" || extension === "pdf";
    ...
    // Hash-based identifiers and data URIs
    const parsed = fileService.parseDataURI(fileURI);
    if (!parsed) {
        return (
            <div className={`mt-2 ${className}`}>
                <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sm font-medium text-indigo-600 hover:text-indigo-800 hover:underline inline-flex items-center gap-1"
                >
                    {title || "View Document"}
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
    ...
    const { dataURI, mimeType, extension, hash } = parsed;
            ...
            <div className={`mt-2 ${className}`}>
                <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p className="text-sm text-yellow-800">
                        ⚠️ File data not available. The file may have been cleared from browser storage.
                    </p>
                    <p className="text-xs text-yellow-600 mt-1">
                        File hash: {hash.substring(0, 16)}...
                    </p>
                </div>
            </div>
        );
                ...
                <div className="mt-3 p-3 bg-gray-50 border border-gray-300 rounded-lg">
                    <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm text-gray-700">
                            Document available. Click the link above to view or
                            download.
                        </p>
                    </div>
                </div>
